...
constrained.smoothspline <- function(y,
  ...
  if(num.folds=="leave-one-out"){
    ...
    if(num.folds>1){
      folds_tmp <- cvTools::cvFolds(length(times), K=num.folds, type="interleaved")
  ...
  # intialize basis
  basis <- fda::create.bspline.basis(norder=order_splines, breaks=times)
  penmat <- fda::getbasispenalty(basis,Lfdobj=pen.degree)
  ...
  if(!is.numeric(lambda)){
    ...
    for(i in 1:num.folds){
      ...
      Bmat <- fda::getbasismatrix(times[train], basis, nderiv=0, returnMatrix=FALSE)
      Bmat.val[[i]] <- fda::getbasismatrix(times[validation[[i]]], basis, nderiv=0, returnMatrix=FALSE)
      Bmat.deriv <- fda::getbasismatrix(times[train], basis, nderiv=1, returnMatrix=FALSE)
      dvec[[i]] <- train_y %*% Bmat
  ...
  Bmat <- fda::getbasismatrix(times, basis, nderiv=0, returnMatrix=FALSE)
  ...
  cost_function <- function(spar){
    ...
    for(i in 1:num.folds){
      ...
      solutions_qp <- quadprog::solve.QP(Dmatsc,
                               dvec[[i]]/sc,
                               Amat[[i]],
                               bvec[[i]],
                               meq[i])
  ...
  Bmat <- fda::getbasismatrix(times, basis, nderiv=0, returnMatrix=FALSE)
  Bmat.deriv <- fda::getbasismatrix(times, basis, nderiv=1, returnMatrix=FALSE)
  dvec <- t(y %*% Bmat)
  Dmat_1 <- t(Bmat) %*% Bmat
  Dmat_2 <- lambda * penmat
  Dmat <- Dmat_1 + Dmat_2
  ...
  Bmat.new <- fda::getbasismatrix(times.new, basis, nderiv=0, returnMatrix=FALSE)
